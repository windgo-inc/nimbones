...
task tests, "Running all tests":
  ...
  exec "wgmkpdf 'Module Unit Tests' \"`cat tmp_filename`\"  \"`cat tmp_filename`.pdf\" || echo 'Failed to generate PDF from test results!'"
  echo "done."
...
task bench, "Running benchmarks":
  ...
  echo ""
  echo "Generating PDF..."
  exec "wgmkpdf 'Module Benchmark Results' \"`cat tmp_filename`\"  \"`cat tmp_filename`.pdf\" || echo 'Failed to generate PDF from benchmark results!'"
  echo "done."
  exec "rm tmp_filename"
